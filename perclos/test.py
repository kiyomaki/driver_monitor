...
def main():
    ...
    while True:
        ...
        current_time = datetime.now().strftime("%H:%M:%S")  # 現在時刻をhh:mm:ss形式で取得
        ...
        if time.time() - last_output_time >= 1:
            if len(perclos_accumulator) > 0:
                ...
                csv_handler.append_row(current_time, round(average_perclos, 3), current_state)
                print(f"[Output] Elapsed Seconds: {elapsed_seconds}, Current Time: {current_time}, Average PERCLOS: {round(average_perclos, 3)}, State: {current_state}")
            else:
                csv_handler.append_row(current_time, "", "")  # 空欄の行を出力
                print(f"[Output] Elapsed Seconds: {elapsed_seconds}, Current Time: {current_time}, No Data")
